...
class PolyMinvFit:
    ...
    def __init__(self, path, par_str, poly_deg, guesses, monitor=True, **kwargs):
        ...
        if 'flow_case' in kwargs.keys():
            ...
            self.flow_case = 'v1'
        if 'ybin' in kwargs.keys() and 'cen' in kwargs.keys():
            self.ybin = kwargs['ybin']
            self.cen = kwargs['cen']
            if self.ptbin == [1]:
                self.ratio = 1
            y_vals = np.sum(np.array([file[f'h{self.par_str}M_cen_y_{i - 1}_{j}'].values()
                                        for i in self.cen for j in self.ybin]), axis=0)
            y_errs = np.sum(np.array([file[f'h{self.par_str}M_cen_y_{i - 1}_{j}'].errors()
                                        for i in self.cen for j in self.ybin]) ** 2, axis=0) ** 0.5
            v2_hists = [
                file[f"h{self.par_str}_EPD_{self.flow_case}_{i - 1}_{j}"] for i in self.cen for j in self.ybin]
    ...
    def fit_v2(self, masked=False, v2=0.05, a=0, b=0, save_to=None, cen=None, ybin=None):
        ...
        if self.monitor or save_to is not None:
            ...
            y_lb, y_rb = -999, -999
            if type(self.ybin) is not int:
                y_lb = self.ybin[0] * 0.1 - 1
                y_rb = (self.ybin[-1]+1) * 0.1 - 1
            elif self.ybin != -1:
                y_lb = self.ybin * 0.1 - 1
                y_rb = (self.ybin+1) * 0.1 - 1
            ax.annotate(fr'{y_lb:.1f}<y<{y_rb:.1f}', xy=(0.1, 0.8),
                            fontsize=15, xycoords='axes fraction')
            ...
            if self.monitor:
                plt.show()
                plt.close(fig)
            if save_to is not None and cen is not None and ybin is not None:
                dict_out = {
                    'x': self.x_vals_v2,
                    'y': self.v2_vals, 
                    'yerr': self.v2_errs,
                    'background': b_over_total(self.x_vals_v2) * (m.values['p0'] + m.values['p1'] * (self.x_vals_v2 - self.hyperon_mass)),
                    'signal': (1 - b_over_total(self.x_vals_v2)) * m.values['v2'],
                    'total': func(self.x_vals_v2, *m.values),
                    'cen_string': cen_string,
                    'y_string': fr'{y_lb:.1f}<y<{y_rb:.1f}'
                }
                for key, val in dict_out.items():
                    if type(val) is np.ndarray:
                        dict_out[key] = val.tolist()

                # --- NEW BLOCK ---
                cen_string_orig = save_to.split('/')[-1].split('_')[-2]
                y_string_orig = save_to.split('/')[-1].split('_')[-1].split('.yaml')[0]
                
                specific_save_to = save_to.replace(cen_string_orig, f'cen{cen[0]}').replace(y_string_orig, f'y{y_lb:.1f}{y_rb:.1f}')
                specific_v1fit_save_to = specific_save_to.replace('invmass', 'v1fit')
                
                path = Path(specific_v1fit_save_to).parent
                path.mkdir(parents=True, exist_ok=True)
                
                with open(specific_v1fit_save_to, 'w') as f:
                    yaml.dump(dict_out, f, default_flow_style=False) 
                    
                # Dynamically write EXACTLY what Snakemake passed in
                if y_lb < 0.72 and y_rb > 0.72 and cen[0] == 4 and (y_string_orig in ['y0.7', 'y0.4']):
                    snakemake_v1fit_save_to = save_to.replace('invmass', 'v1fit')
                    with open(snakemake_v1fit_save_to, 'w') as f:
                        yaml.dump(dict_out, f, default_flow_style=False)

                # plt.tight_layout()
                # ax.set_xlim(self.hyperon_mass - 0.01, self.hyperon_mass + 0.01)
                # print("Saving to", save_to.replace('invmass', 'v1fit'))
                # pickle.dump(fig, open(save_to.replace('invmass', 'v1fit'), 'wb'))
                # plt.close(fig)
    ...
    def plot(self, save_to=None, cen=None, ybin=None, ybin_total=None):
        ...
        y_lb, y_rb = -999, -999
        if type(self.ybin) is not int:
            y_lb = self.ybin[0] * 2. / ybin_total - 1
            y_rb = (self.ybin[-1]+1) * 2. / ybin_total - 1
        elif self.ybin != -1:
            y_lb = self.ybin * 2. / ybin_total - 1
            y_rb = (self.ybin+1) * 2. / ybin_total - 1
        plt.annotate(fr'{y_lb:.1f}<y<{y_rb:.1f}', xy=(0.1, 0.8),
                        fontsize=15, xycoords='axes fraction')
        if self.ptbin != -1:
        ...
        if save_to is not None and cen is not None and ybin is not None:
            dict_out = {
                ...
                'xfit': x_smooth[:-1],
                'yerr': self.y_errs, 
                'background': y_bg * self.ratio, 
                'signal': y_fitted_vals - y_bg_sparse, 
                'total': y_exp * self.ratio,
                'cen_string': cen_string,
                'y_string': fr'{y_lb:.1f}<y<{y_rb:.1f}',
                'S': self.signal,
                'S_error': self.signal_error,
                'S+B': total,
                'sigma': sigma
            }
            for key, val in dict_out.items():
            ...
            path = Path(specific_save_to).parent
            path.mkdir(parents=True, exist_ok=True)

            with open(specific_save_to, 'w') as f:
                yaml.dump(dict_out, f, default_flow_style=False)
            ...
            # pickle.dump(fig, open(save_to, 'wb'))
...
def main(path, outputDir, particle_str, flow_case, yrebin=1, max_refit=500, paper_plot_path=None, **kwargs):
    start = time.time()
    monitor = 0
    paper_plot_cen = 4
    paper_plot_y = 2
    # Extract energy dynamically by finding the chunk with 'GeV'
    filename_only = path.split('/')[-1]
    energy_str = next((p for p in filename_only.split('_') if 'GeV' in p), '19p6GeV').replace('.root', '')
    ...
    if energy_str in ['7p7GeV', '9p2GeV', '11p5GeV', '14p6GeV', '17p3GeV', '19p6GeV', '27GeV']:
        ybin_hi = 19 # 20 bins at 7.7 GeV

    ### FOR DEBUG MODE
    if 'cen' in kwargs.keys() and 'ybin' in kwargs.keys():
        cen = [[kwargs['cen']]]
        ybin_merged = kwargs['ybin']
        # if 'Xi' in particle_str:
        ybin_lo = yrebin * ybin_merged
        ybin_hi = yrebin * ybin_merged + yrebin - 1
        monitor = 1
        print(f'DEBUG MODE: cen = {cen}, ybin = {ybin_merged}')
        print(f'            particle_str = {particle_str}, flow_case = {flow_case}')
        print(f'            energy_str = {energy_str}')
    
    num_ybin = ybin_hi - ybin_lo + 1
    num_yrebin = yrebin
    v2_cen_y = {c[0]: np.array([None for _ in range(
        ybin_lo, ybin_hi + 1, num_yrebin)]) for c in cen}
    ...
    with std_out_err_redirect_tqdm() as orig_stdout:
        ...
        for c in cen:
            for i, ybin in enumerate(np.arange(ybin_lo, ybin_hi + 1).reshape(num_ybin // num_yrebin, num_yrebin)):
                if 'cen' in kwargs.keys() and 'ybin' in kwargs.keys(): # DEBUG MODE
                    print(f'DEBUG MODE: c = {c}, ybin = {ybin}')
                ...
                fitter = PolyMinvFit(path=path, par_str=particle_str, poly_deg=poly_deg,
                                     ...
                                     cen=c, ybin=ybin, flow_case=flow_case)
                invm_fitted = fitter.fit_iminuit(masked=True)
                save_path = None
                if paper_plot_path is not None and invm_fitted: # and c == paper_plot_cen and i == paper_plot_y:
                    save_path = paper_plot_path
                    fitter.plot(save_to=save_path, cen=c, ybin=ybin, ybin_total=num_ybin)
        ...
        refit_counter = {c[0]: [0 for _ in range(
            (ybin_hi+1-ybin_lo) // num_yrebin)] for c in cen}
        strategies = {c[0]: [None for _ in range(
            (ybin_hi+1-ybin_lo) // num_yrebin)] for c in cen} # 1 if fixed z, None if not
        while has_badfit:
            for c in cen:
                ybins = missing_bin[c[0]]
                for i, ybin in enumerate(ybins):
                    ...
                    if refit_counter[c[0]][i] > max_refit:
                        ...
                        print(
                            f'cen = {c}, ybin = {ybin} is not fitted after {max_refit} trials, skipping ...')
                        pbar2.update(1)
                    guesses = [  # .05, 1.67272, 0.002, # gaus
                        np.random.uniform(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--paper_plot_path', type=str, default=None,
                       help='path to save paper plots')
    parser.add_argument('--debug', action='store_true', help='debug mode')
    parser.add_argument('--cen_bin', type=int, required='--debug' in sys.argv, help='central bin')
    parser.add_argument('--y_bin', type=int, required='--debug' in sys.argv, help='y bin')
    ...
    if args.debug:
        kw_dict = {'cen': args.cen_bin, 'ybin': args.y_bin}
    else:
        kw_dict = {}
    main(inputDir, outputDir, particle_str, flow_case,
         yrebin=args.yrebin, max_refit=args.max_refit, paper_plot_path=args.paper_plot_path, **kw_dict)
